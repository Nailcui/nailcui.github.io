...
> 当CGO_ENABLED=1编译时，会将文件中引用的libc的库，已动态连接的方式生成目标文件，这样就要求运行环境必须有这个动态链接库
>
> 当CGO_ENABLED=0编译时，会把目标文件中未定义的符号、外部函数一起链接到可执行文件中，这样可以最小化对环境的依赖。
>
> -ldflags="-w -s" 去掉调试信息，可以减小构建后文件体积
>
> -ldflags="-extldflags=-static -w -s" 静态编译，去掉调试信息
